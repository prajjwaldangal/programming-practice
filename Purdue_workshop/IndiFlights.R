# class
# sum
# names, cut, plot, head, table, na.rm, 

# Prajjwal Dangal

# all data imported from 2008 data set of 2009 ASA Data Expo
myDF <- read.csv('/Users/prajjwaldangal/Documents/cs/summer2018/2008.csv')

head(myDF)

# here's a smaller dataframe with *only* flights whose origin is Indianapolis
myIndyOrigins <- subset(myDF, myDF$Origin == "IND")

# trying to store table with month based representation of flights' info:
# 1                       2                       3                         4
# no. of flights in Jan   no. of flights in Feb   no. of flights in March   and so on...
tWithMonth <- table(myIndyOrigins$Month)

# plot the table that contains the smaller dataframe
plot(table(myIndyOrigins$Month))

head(myDF)
# this is the number of flights that departed before or at time 24:00 
# 42011
sum(myIndyOrigins$DepTime <= 2400, na.rm = TRUE)
# na.rm means to remove any values that were unknown

# find the number of flights whose departure times were unknown
sum(is.na(myIndyOrigins$DepTime))


########################## upto Analyzing the Departure Times of Flights  #######################
...
# 0, 100, 200, ..., 2400 i.e. 100 units of distance
# between one another
seq(0, 2400, by=100)
# group each of the 7009728 depTime entries into its
# corresponding bin of width 100
cut(myDF$DepTime, breaks=seq(0,2400,by=100))

# group into bins of width 100, similar to above 
...
# paste(a, b) vectorized, makes string tuple of members of a and
# b
head(paste(myDF$Origin, myDF$Dest))
tail((paste(myDF$Origin, " to ", myDF$Dest)),20)
sum(paste(myDF$Origin, myDF$Dest), na.rm=TRUE)


####### week three #######
# Most popular airports
tail(sort(table(myDF$Origin), decreasing=T))[1:8]
?sort

# tail(sort(table(myDF$Origin), decreasing=T))[1:8]
# 
# GST  BJI  INL  TUP  PIR  PUB <NA> <NA> 
#   85   74   72   10    5    2          
...
# 10 least popular airports in 2008
leastPop <- names(sort(table(myDF$Dest), decreasing=F)[1:10]) 
head(leastPop)
...
table(myDF$Origin %in% mostpopular & myDF$Dest %in% mostpopular)
...
plot(table(cut(myDF$DepTime, breaks=seq(0,2400,100))))
